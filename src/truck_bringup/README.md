...
The `truck_bringup` package is an example bringup system for Nav2 applications.
...
```bash
source /opt/ros/dashing/setup.bash
ros2 launch truck_bringup bringup_launch.py use_sim_time:=True autostart:=True \
...
```bash
source /opt/ros/dashing/setup.bash
ros2 run rviz2 rviz2 -d $(ros2 pkg prefix truck_bringup)/share/truck_bringup/launch/nav2_default_view.rviz
```
...
```bash
ros2 launch truck_bringup tb3_simulation_launch.py <settings>
```
...
```bash
ros2 launch truck_bringup multi_tb3_simulation_launch.py <settings>
```
...
* _Please note that currently, truck_bringup works if you provide a map file. However, providing a map is not required to use Nav2. Nav2 can be configured to use the costmaps to navigate in an area without using a map file_
...
```bash
source /opt/ros/dashing/setup.bash
ros2 launch truck_bringup bringup_launch.py map:=<full/path/to/map.yaml> map_type:=occupancy
```
